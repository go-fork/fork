...
// NewWebAppWithConfig tạo một HTTP WebApp với config từ provider
// Đây là helper function để tạo WebApp với config đã được load sẵn
//
// Parameters:
//   - configProvider: Config provider để load cấu hình
//   - configKey: Key để load config từ provider (mặc định: "http")
//
// Returns:
//   - *WebApp: HTTP WebApp đã được cấu hình
//   - error: Lỗi nếu có trong quá trình load config
func NewWebAppWithConfig(configProvider interface{}, configKey string) (*WebApp, error) {
	if configKey == "" {
		configKey = "http"
	}

	// Load config từ provider
	appConfig, err := LoadConfigFromProvider(configProvider, configKey)
	if err != nil {
		return nil, err
	}
	...
	// Setup graceful shutdown nếu được enabled
...
// ReloadWebAppConfig reload lại config cho HTTP WebApp từ provider
// Đây là helper function để reload config mà không cần restart WebApp
//
// Parameters:
//   - configProvider: Config provider để load cấu hình mới
//   - configKey: Key để load config từ provider (mặc định: "http")
//
// Returns:
//   - error: Lỗi nếu có trong quá trình reload config
func (app *WebApp) ReloadWebAppConfig(configProvider interface{}, configKey string) error {
	if configKey == "" {
		configKey = "http"
	}

	// Load config mới từ provider
	newConfig, err := LoadConfigFromProvider(configProvider, configKey)
	if err != nil {
		return err
	}
